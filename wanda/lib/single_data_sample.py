...
"""
def get_streaming_dataset(tokenizer, data, field_name, nsamples, seqlen, seed):        
    random.seed(seed)
    
    shuffled_data = data.shuffle(buffer_size=20000, seed=seed)
    dataset_head = shuffled_data.take(20000)
    dataset = list(dataset_head)
    
    list_samples = []
    for i in range(nsamples):
        list_samples.append(dataset[i][field_name])
        

    #tokenized_input = tokenizer(list_samples[0], return_tensors='pt', padding='max_length', truncation=True, max_length=seqlen)
    tokenized_input = tokenizer(list_samples[0:2], return_tensors='pt', padding='longest', truncation=True, max_length=seqlen)
...
def get_streaming_dataset(tokenizer, data, field_name, nsamples, seqlen, seed):        
    ...
    for sample in range(nsamples):
        n_Data = 0
        while True:
            trainenc = tokenizer(dataset[indice][field_name], return_tensors='pt')
            indice += 1
            n_Data += 1
        ...
        list_samples.append(inp)
    
    #torch_list_samples = torch.stack(list_samples, dim=0).squeeze(1)
    #print("torch_list_samples.shape", torch_list_samples.shape)
...
def get_single_c4(nsamples, seed, seqlen, tokenizer): #It works
    data = load_dataset('allenai/c4', "en", split='train', streaming=True)
    return get_streaming_dataset(tokenizer, data, "text", nsamples, seqlen, seed)


def get_single_wikitext(nsamples, seed, seqlen, tokenizer):
    random.seed(seed)
...
def get_single_pile(nsamples, seed, seqlen, tokenizer):
    data = load_dataset('monology/pile-uncopyrighted', data_files={'train': 'train/00.jsonl.zst'}, split='train', streaming=True)
    return get_streaming_dataset(tokenizer, data, "text", nsamples, seqlen, seed)


def get_single_oscar(nsamples, seed, seqlen, tokenizer):
    dataset = load_dataset('oscar-corpus/OSCAR-2301', name='en',  split='train', streaming=True, trust_remote_code=True)
    data = dataset.shard(num_shards=2, index=0)
    return get_streaming_dataset(tokenizer, data, "text", nsamples, seqlen, seed)


def get_single_red_pajama(nsamples, seed, seqlen, tokenizer): #It works
    data = load_dataset('togethercomputer/RedPajama-Data-V2',  name='default',  languages=["en"], snapshots=["2014-23"], streaming=True)["train"]
    return get_streaming_dataset(tokenizer, data, "raw_content", nsamples, seqlen, seed)
...
def get_opc(nsamples, seed, seqlen, tokenizer, rationale=False, input_format='concat', padding_side='left', mode='llama', verbose=False, train=True):
    ...
    traindata = load_dataset("OpenCoder-LLM/opc-sft-stage2", "educational_instruct", split="train[:100000]") #load_dataset('iwslt2017', 'iwslt2017-en-fr', split='train', verification_mode='no_checks', trust_remote_code=True)
    traindata = traindata.map(prepare_input)
    ...
    if train:
        ...
        testdata = load_dataset("OpenCoder-LLM/opc-sft-stage2", "educational_instruct", split="train[100000:]")
        test_prompt = testdata[:]['prompt']
        test_answer = testdata[:]['answer']
...
def get_oci(nsamples, seed, seqlen, tokenizer, rationale=False, input_format='concat', padding_side='left', mode='llama', verbose=False, train=True):
    def prepare_input(example):
        #translation = example['translation']
        #print('type(translation)', type(translation))
        #english_version = translation["en"]
    ...
    traindata = load_dataset("m-a-p/CodeFeedback-Filtered-Instruction", split="train[:120000]") #load_dataset('iwslt2017', 'iwslt2017-en-fr', split='train', verification_mode='no_checks', trust_remote_code=True)
    traindata = traindata.map(prepare_input)
    ...
    if train:
        ...
        testdata = load_dataset("m-a-p/CodeFeedback-Filtered-Instruction", split="train[120000:]")
        test_prompt = testdata[:]['prompt']
        test_answer = testdata[:]['answer']
...
def get_tpa(nsamples, seed, seqlen, tokenizer, rationale=False, input_format='concat', padding_side='left', mode='llama', verbose=False, train=True):
    def prepare_input(example):
        #translation = example['translation']
        #print('type(translation)', type(translation))
        #english_version = translation["en"]
    ...
    traindata = load_dataset("Vezora/Tested-143k-Python-Alpaca", split="train[:120000]") #load_dataset('iwslt2017', 'iwslt2017-en-fr', split='train', verification_mode='no_checks', trust_remote_code=True)
    traindata = traindata.map(prepare_input)
    ...
    if train:
        ...
        testdata = load_dataset("Vezora/Tested-143k-Python-Alpaca", split="train[120000:]")
        test_prompt = testdata[:]['prompt']
        test_answer = testdata[:]['answer']
...
def get_single_loaders(name, nsamples=128, seed=0, seqlen=2048, data_seqlen=None, tokenizer=None, rationale=False, input_format='single', difficulty=1, padding_side='left', mode='llama', num_incontext=None, num_cot_steps=None, verbose=False, train=True):
    if 'wikitext2' in name:
        return get_single_wikitext(nsamples, seed, seqlen, tokenizer)
    
    if "c4" in name:
        return get_single_c4(nsamples, seed, seqlen,  tokenizer)
    
    if "oscar" in name:
        return get_single_oscar(nsamples, seed, seqlen, tokenizer)
    
    if "redpajama" in name:
        return get_single_red_pajama(nsamples, seed, seqlen, tokenizer)
    
    if "pile" in name:
        return get_single_pile(nsamples, seed, seqlen, tokenizer)
    ...
    if 'oci' in name:
        return get_oci(nsamples, seed, seqlen, tokenizer, input_format=input_format, rationale=rationale, padding_side=padding_side, mode=mode, verbose=verbose, train=train)

    if 'tpa' in name:
        return get_tpa(nsamples, seed, seqlen, tokenizer, input_format=input_format, rationale=rationale, padding_side=padding_side, mode=mode, verbose=verbose, train=train)
