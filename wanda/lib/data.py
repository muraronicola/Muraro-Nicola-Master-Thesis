...
def get_streaming_dataset(tokenizer, traindata, valdata, field_name, nsamples, seqlen, seed, train=True):        
    random.seed(seed)
    
    if train:
        shuffled_traindata = traindata.shuffle(buffer_size=50000, seed=seed)  #Per 1024 samples su c4: 50000. Old value era 20000
        dataset_head_train = shuffled_traindata.take(50000)
        dataset_train = list(dataset_head_train)
        ...
        for sample in range(nsamples):
            n_Data = 0
            while True:
                trainenc = tokenizer(dataset_train[indice][field_name], return_tensors='pt')
                indice += 1
                n_Data += 1
                if indice >= len(dataset_train):
                    print("Asking for new data....")
                    raise Exception("Asking for new data....")
                    #indice = 0
                    #shuffled_traindata = traindata.shuffle(buffer_size=10000, seed=42*sample)
                    #dataset_head_train = shuffled_traindata.take(10000)
                    #dataset_train = list(dataset_head_train)
                
                if trainenc.input_ids.shape[1] > seqlen:
                    break
            
            #print(f"Sampled {sample} from training data")
        ...
        shuffled_valdata = valdata.shuffle(buffer_size=20000, seed=seed)
        dataset_head_val = shuffled_valdata.take(1024)
        dataset_val= list(dataset_head_val)
        
        # Prepare validation dataset
        joined_texts = ' '.join(d[field_name] for d in dataset_val)# era 1100
        valenc = tokenizer(joined_texts, return_tensors='pt')# era 1100
        valenc = valenc.input_ids[:, :(256 * seqlen)]
        valenc = TokenizerWrapper(valenc)
        return None, valenc, None


# Load and process wikitext2 dataset
def get_wikitext2(nsamples, seed, seqlen, tokenizer, train=True):
    random.seed(seed)
    
    if train:
        traindata = load_dataset('wikitext', 'wikitext-2-raw-v1', split='train[:10000]')
        ...
        testenc = tokenizer("\n\n".join(testdata['text']), return_tensors='pt') #, max_length=seqlen, truncation=True)
        return None, testenc, None



# Load and process c4 dataset
def get_c4(nsamples, seed, seqlen, tokenizer, train=True): #It works
    ...
    return get_streaming_dataset(tokenizer, traindata, valdata, 'text', nsamples, seqlen, seed, train)       



def get_pile(nsamples, seed, seqlen, tokenizer, train=True):
    
    # Load train and validation datasets
    #traindata = load_dataset('monology/pile-uncopyrighted', data_files={'train': 'train/00.jsonl.zst'}, split='train[:5000]', verification_mode='no_checks')
    #valdata = load_dataset('monology/pile-uncopyrighted', data_files={'train': 'train/00.jsonl.zst'}, split='train[5000:5100]', verification_mode='no_checks')
    
    # Load train and validation datasets
    traindata = load_dataset('monology/pile-uncopyrighted', data_files={'train': 'train/00.jsonl.zst'}, split='train', streaming=True)
    valdata = load_dataset('monology/pile-uncopyrighted', data_files={'eval': 'val.jsonl.zst'}, split='eval', streaming=True)
    
    return get_streaming_dataset(tokenizer, traindata, valdata, 'text', nsamples, seqlen, seed, train)       



def get_oscar(nsamples, seed, seqlen, tokenizer, train=True):

    dataset = load_dataset('oscar-corpus/OSCAR-2301', name='en',  split='train', streaming=True, trust_remote_code=True)
    
    traindata = dataset.shard(num_shards=2, index=0)
    valdata = dataset.shard(num_shards=2, index=1)
    
    return get_streaming_dataset(tokenizer, traindata, valdata, 'text', nsamples, seqlen, seed, train)       


def get_red_pajama(nsamples, seed, seqlen, tokenizer, train=True): #It works
    
    traindata = load_dataset('togethercomputer/RedPajama-Data-V2',  name='default',  languages=["en"], snapshots=["2014-23"], streaming=True)["train"]
    valdata = load_dataset('togethercomputer/RedPajama-Data-V2',  name='default',  languages=["en"], snapshots=["2014-15"], streaming=True)["train"]

    return get_streaming_dataset(tokenizer, traindata, valdata, 'raw_content', nsamples, seqlen, seed, train)       


def get_gsm8k(nsamples, seed, seqlen, tokenizer, rationale=False, padding_side='left', input_format='single', mode='llama', num_incontext=None, num_cot_steps=None, verbose=False, train=True):
    def prepare_input(example):
    ...
    if train:
        ...
        return trainloader, (None, None), pad_token
    
    else:
        testdata = load_dataset('gsm8k', 'main', split='test', verification_mode='no_checks')
        testdata = testdata.map(prepare_input)

        test_prompt = testdata[:]['prompt'] #nsamples
        test_answer = testdata[:]['label'] #nsamples

        return None, (test_prompt, test_answer), pad_token


def get_svamp_rationale(nsamples, seed, seqlen, tokenizer, input_format='single', padding_side='left', mode='llama', verbose=False, train=True):
...
def get_svamp_individual(tokenizer, split='train', train=True):
    def prepare_input(example):
...
def get_svamp(nsamples, seed, seqlen, tokenizer, rationale=False, input_format='concat', padding_side='left', mode='llama', verbose=False, train=True):
    if rationale:
...
def get_anli(nsamples, seed, seqlen, tokenizer, difficulty = 1, rationale=False, input_format='concat', padding_side='left', mode='llama', verbose=False, train=True):
    ...
    traindata = load_dataset('anli', split='train_r{}'.format(difficulty), verification_mode='no_checks', revision=version_commit_sha, trust_remote_code=True)
    traindata = traindata.map(prepare_input)
    ...
    if train:
        ...
        testdata = load_dataset('anli', split='test_r{}'.format(difficulty), verification_mode='no_checks', revision=version_commit_sha, trust_remote_code=True)
        testdata = testdata.map(prepare_input)
...
def get_esnli(nsamples, seed, seqlen, tokenizer, rationale=False, input_format='concat', padding_side='left', mode='llama', verbose=False, train=True):
    classes = ['entailment', 'neutral', 'contradiction']
    def prepare_input(example):
        premise = example['premise']
        hypothesis = example['hypothesis']
    ...
    traindata = load_dataset('esnli', split='train', verification_mode='no_checks', trust_remote_code=True)
    traindata = traindata.map(prepare_input)
    ...
    if train:
        ...
        testdata = load_dataset('esnli', split='test', verification_mode='no_checks', trust_remote_code=True)
        testdata = testdata.map(prepare_input)
...
def get_esnli_old(nsamples, seed, seqlen, tokenizer, rationale=False, input_format='concat', padding_side='left', mode='llama', verbose=False, train=True):
...
def get_mawps(nsamples, seed, seqlen, tokenizer, rationale=False, input_format='concat', padding_side='left', mode='llama', verbose=False, train=True):
    def prepare_input(example):
        question = example['question']
...
def get_rte(nsamples, seed, seqlen, tokenizer, rationale=False, input_format='concat', padding_side='left', mode='llama', verbose=False, train=True):
    def prepare_input(example):
    ...
    traindata = load_dataset('yangwang825/rte', 'default', split='train', verification_mode='no_checks')
    traindata = traindata.map(prepare_input)
    ...
    if train:
        ...
        testdata = load_dataset('yangwang825/rte', 'default', split='test', verification_mode='no_checks')
        testdata = testdata.map(prepare_input)
        test_prompt = testdata[:]['prompt']
        test_answer = testdata[:]['answer']
...
def get_boolq(nsamples, seed, seqlen, tokenizer, rationale=False, input_format='concat', padding_side='left', mode='llama', verbose=False, train=True):
    def prepare_input(example):
    ...
    traindata = load_dataset('boolq', 'default', split='train', verification_mode='no_checks')
    traindata = traindata.map(prepare_input)
    ...
    if train:
        ...
        testdata = load_dataset('boolq', 'default', split='validation', verification_mode='no_checks')
        testdata = testdata.map(prepare_input)
...
def get_commonsense_qa(nsamples, seed, seqlen, tokenizer, rationale=False, input_format='concat', padding_side='left', mode='llama', verbose=False, train=True):
    def prepare_input(example):
        question = example['question']
...
def get_race(nsamples, seed, seqlen, tokenizer, rationale=False, input_format='concat', padding_side='left', mode='llama', verbose=False, train=True):
    def prepare_input(example):
...
def get_winogrande(nsamples, seed, seqlen, tokenizer, rationale=False, input_format='concat', padding_side='left', mode='llama', verbose=False, train=True):
    def prepare_input(example):
    ...
    traindata = load_dataset('winogrande', 'winogrande_m', split='train', verification_mode='no_checks', trust_remote_code=True)
    traindata = traindata.map(prepare_input)
    ...
    if train:
        ...
        testdata = load_dataset('winogrande', 'winogrande_m', split='validation', verification_mode='no_checks', trust_remote_code=True)
        testdata = testdata.map(prepare_input)
        test_prompt = testdata[:]['prompt']
        test_answer = testdata[:]['answer']
...
def get_wmt14(nsamples, seed, seqlen, tokenizer, rationale=False, input_format='concat', padding_side='left', mode='llama', verbose=False, train=True):
    def prepare_input(example):
    ...
    traindata = load_dataset('wmt/wmt14', 'fr-en', split='train', data_files={"train": "fr-en/train-00000-of-00030.parquet"},  verification_mode='no_checks')
    traindata = traindata.map(prepare_input)
    ...
    if train:
        ...
        testdata = load_dataset('wmt/wmt14', 'fr-en', split='test', data_files={"test": "fr-en/test-00000-of-00001.parquet"},  verification_mode='no_checks')
        testdata = testdata.map(prepare_input)
        test_prompt = testdata[:]['prompt']
        test_answer = testdata[:]['answer']
...
def get_iwslt(nsamples, seed, seqlen, tokenizer, rationale=False, input_format='concat', padding_side='left', mode='llama', verbose=False, train=True):
    def prepare_input(example):
        translation = example['translation']
        #print('type(translation)', type(translation))
        english_version = translation["en"]
    ...
    traindata = load_dataset('iwslt2017', 'iwslt2017-en-fr', split='train', verification_mode='no_checks', trust_remote_code=True)
    traindata = traindata.map(prepare_input)
    ...
    if train:
        ...
        testdata = load_dataset('iwslt2017', 'iwslt2017-en-fr', split='test', verification_mode='no_checks', trust_remote_code=True)
        testdata = testdata.map(prepare_input)
        test_prompt = testdata[:]['prompt']
        test_answer = testdata[:]['answer']
...
def get_opc(nsamples, seed, seqlen, tokenizer, rationale=False, input_format='concat', padding_side='left', mode='llama', verbose=False, train=True):
    def prepare_input(example):
        #translation = example['translation']
        #print('type(translation)', type(translation))
        #english_version = translation["en"]
    ...
    traindata = load_dataset("OpenCoder-LLM/opc-sft-stage2", "educational_instruct", split="train[:117000]") #118,278
    traindata = traindata.map(prepare_input)
    
    if padding_side == 'left':
        pad_token = tokenizer(traindata[0]['prompt'], return_tensors='pt', padding='max_length', max_length=seqlen).input_ids[0][0]
    elif padding_side == 'right':
        pad_token = tokenizer(traindata[0]['prompt'], return_tensors='pt', padding='max_length', max_length=seqlen).input_ids[0][-1]
    #print('pad_token', pad_token)
    ...
    if train:
        ...
        testdata = load_dataset("OpenCoder-LLM/opc-sft-stage2", "educational_instruct", split="train[117000:]")
        testdata = testdata.map(prepare_input)
        test_prompt = testdata[:]['prompt']
        test_answer = testdata[:]['answer']
...
def get_ds1000(nsamples, seed, seqlen, tokenizer, rationale=False, input_format='concat', padding_side='left', mode='llama', verbose=False, train=True):
    def prepare_input(example):
        #translation = example['translation']
        #print('type(translation)', type(translation))
        #english_version = translation["en"]
        ...
        example['answer'] = example['reference_code']
        example['prompt_answer'] = example['prompt'] + example['answer']
        
        return example

    traindata = load_dataset("code-rag-bench/ds1000", split="train[:800]") # 1000
    traindata = traindata.map(prepare_input)
    ...
    if train:
        ...
        testdata = load_dataset("code-rag-bench/ds1000", split="train[800:]")
        testdata = testdata.map(prepare_input)
        test_prompt = testdata[:]['prompt']
        test_answer = testdata[:]['answer']
...
def get_mbpp(nsamples, seed, seqlen, tokenizer, rationale=False, input_format='concat', padding_side='left', mode='llama', verbose=False, train=True):
    def prepare_input(example):
        #translation = example['translation']
        #print('type(translation)', type(translation))
        #english_version = translation["en"]
        ...
        example['answer'] = example['code']
        example['prompt_answer'] = example['prompt'] + example['answer']
        
        return example

    #file: data-00000-of-00009
    traindata = load_dataset("Muennighoff/mbpp", split="test[:800]", trust_remote_code=True) 
    traindata = traindata.map(prepare_input)
    ...
    if train:
        ...
        testdata = load_dataset("Muennighoff/mbpp", split="test[800:]", trust_remote_code=True) 
        testdata = testdata.map(prepare_input)
...
def get_loaders(name, nsamples=128, seed=0, seqlen=2048, data_seqlen=None, tokenizer=None, rationale=False, input_format='single', difficulty=1, padding_side='left', mode='llama', num_incontext=None, num_cot_steps=None, verbose=False, train=True):
    if 'wikitext2' in name:
        return get_wikitext2(nsamples, seed, seqlen, tokenizer, train=train)
    
    if "c4" in name:
        return get_c4(nsamples, seed, seqlen,  tokenizer, train=train)
    
    if "oscar" in name:
        return get_oscar(nsamples, seed, seqlen, tokenizer, train=train)
    
    if "redpajama" in name:
        return get_red_pajama(nsamples, seed, seqlen, tokenizer, train=train)
    
    if "pile" in name:
        return get_pile(nsamples, seed, seqlen, tokenizer, train=train)
    
    if 'gsm8k' in name:
        return get_gsm8k(nsamples, seed, seqlen, tokenizer, input_format=input_format, rationale=rationale, padding_side=padding_side, mode=mode, num_incontext=num_incontext, num_cot_steps=num_cot_steps, verbose=verbose, train=train)
    
    if 'svamp' in name:
        return get_svamp(nsamples, seed, seqlen, tokenizer, input_format=input_format, rationale=rationale, padding_side=padding_side, mode=mode, verbose=verbose, train=train)
    
    if 'mawps' in name:
        return get_mawps(nsamples, seed, seqlen, tokenizer, input_format=input_format, rationale=rationale, padding_side=padding_side, mode=mode, verbose=verbose, train=train)

    if 'anli' in name:
        return get_anli(nsamples, seed, seqlen, tokenizer, input_format=input_format, rationale=rationale, difficulty=int(name[-1]), padding_side=padding_side, mode=mode, verbose=verbose, train=train)

    if 'esnli' in name:
        return get_esnli(nsamples, seed, seqlen, tokenizer, input_format=input_format, rationale=rationale, padding_side=padding_side, mode=mode, verbose=verbose, train=train)

    if 'rte' in name:
        return get_rte(nsamples, seed, seqlen, tokenizer, input_format=input_format, rationale=rationale, padding_side=padding_side, mode=mode, verbose=verbose, train=train)

    if 'boolq' in name:
        return get_boolq(nsamples, seed, seqlen, tokenizer, input_format=input_format, rationale=rationale, padding_side=padding_side, mode=mode, verbose=verbose, train=train)

    if 'commonsense_qa' in name:
        return get_commonsense_qa(nsamples, seed, seqlen, tokenizer, input_format=input_format, rationale=rationale, padding_side=padding_side, mode=mode, verbose=verbose, train=train)

    if 'race' in name:
        return get_race(nsamples, seed, seqlen, tokenizer, input_format=input_format, rationale=rationale, padding_side=padding_side, mode=mode, verbose=verbose, train=train)

    if 'winogrande' in name:
        return get_winogrande(nsamples, seed, seqlen, tokenizer, input_format=input_format, rationale=rationale, padding_side=padding_side, mode=mode, verbose=verbose, train=train)

    if 'wmt14' in name:
        return get_wmt14(nsamples, seed, seqlen, tokenizer, input_format=input_format, rationale=rationale, padding_side=padding_side, mode=mode, verbose=verbose, train=train)

    if 'iwslt' in name:
        return get_iwslt(nsamples, seed, seqlen, tokenizer, input_format=input_format, rationale=rationale, padding_side=padding_side, mode=mode, verbose=verbose, train=train)

    if 'opc' in name:
        return get_opc(nsamples, seed, seqlen, tokenizer, input_format=input_format, rationale=rationale, padding_side=padding_side, mode=mode, verbose=verbose, train=train)

    if 'ds1000' in name:
        return get_ds1000(nsamples, seed, seqlen, tokenizer, input_format=input_format, rationale=rationale, padding_side=padding_side, mode=mode, verbose=verbose, train=train)

    if 'mbpp' in name:
        return get_mbpp(nsamples, seed, seqlen, tokenizer, input_format=input_format, rationale=rationale, padding_side=padding_side, mode=mode, verbose=verbose, train=train)
